...
import { seedAdminAuth, mockBooksApi, defaultBooks, type Buch } from './utils/booksTest';
...
const cloneBooks = (books: Buch[]) =>
  books.map((book) => ({
    ...
    abbildungen: book.abbildungen ? book.abbildungen.map((img) => ({ ...img })) : undefined,
  }));
...
test.describe('BookDelete', () => {
  ...
  test('löscht ein Buch', async ({ page }) => {
    ...
    const deleteRequest = page.waitForRequest(
      (request) => request.method() === 'DELETE' && request.url().includes('/rest/1'),
    );

    await deleteButton.click();
    await deleteRequest;

    await expect(page.getByRole('row', { name: /Admin Seed Book/ })).toHaveCount(0);
  });
});
