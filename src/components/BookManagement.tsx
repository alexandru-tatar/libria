...
import { Box, Dialog, DialogTitle, DialogContent, DialogActions, Button, Typography, Fade, Divider } from '@mui/material';
...
export function BookManagement({
  ...
  const closeDialog = () => {
    ...
    try {
      ...
      if (!isbnDigits || isbnDigits.length !== 13) throw new Error('ISBN muss 13 Stellen haben');
  ...
  return (
    <>
      ...
      {/* Erfolg-Dialog */}
      <Dialog open={successDialogOpen} TransitionComponent={Fade} onClose={() => setSuccessDialogOpen(false)} PaperProps={{ sx: { borderRadius: 4, p: 2, maxWidth: 480, mx: 1.5, boxShadow: 12, border: '1px solid rgba(76, 175, 80, 0.2)', position: 'relative' } }}>
        <Box sx={{ position: 'absolute', top: 12, left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(76, 175, 80, 0.12)', color: 'success.main', px: 2, py: 0.5, borderRadius: 999, fontWeight: 600, fontSize: 12, letterSpacing: 0.4 }}>
          ERFOLG
        </Box>
        <DialogTitle sx={{ fontWeight: 700, textAlign: 'center', pt: 5 }}>Alles erledigt</DialogTitle>
        <DialogContent>
          <Typography variant="body1" sx={{ textAlign: 'center', mb: 1.5, color: 'success.main' }}>{successMsg || 'Aktion erfolgreich abgeschlossen.'}</Typography>
          <Divider sx={{ my: 1.5 }} />
          <Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center' }}>Du kannst jetzt ein weiteres Buch anlegen oder das Fenster schließen.</Typography>
        </DialogContent>
        <DialogActions sx={{ justifyContent: 'center', pb: 2 }}>
          <Button variant="contained" onClick={() => setSuccessDialogOpen(false)} sx={{ borderRadius: 999, px: 3 }} color="success">Weiter</Button>
        </DialogActions>
      </Dialog>

      {/* Fehler-Dialog */}
      <Dialog open={errorDialogOpen} TransitionComponent={Fade} onClose={() => setErrorDialogOpen(false)} PaperProps={{ sx: { borderRadius: 4, p: 2, maxWidth: 480, mx: 1.5, boxShadow: 12, border: '1px solid rgba(244, 67, 54, 0.2)', position: 'relative' } }}>
        <Box sx={{ position: 'absolute', top: 12, left: '50%', transform: 'translateX(-50%)', backgroundColor: 'rgba(244, 67, 54, 0.12)', color: 'error.main', px: 2, py: 0.5, borderRadius: 999, fontWeight: 600, fontSize: 12, letterSpacing: 0.4 }}>
          FEHLER
        </Box>
        <DialogTitle sx={{ fontWeight: 700, textAlign: 'center', pt: 5 }}>Hinweis</DialogTitle>
        <DialogContent>
          <Typography variant="body1" sx={{ textAlign: 'center', mb: 1.5, color: 'error.main' }}>{errorMsg || 'Es ist ein Fehler aufgetreten.'}</Typography>
          <Divider sx={{ my: 1.5 }} />
          <Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center' }}>Bitte prüfe deine Eingaben und versuche es erneut.</Typography>
        </DialogContent>
        <DialogActions sx={{ justifyContent: 'center', pb: 2 }}>
          <Button variant="contained" onClick={() => setErrorDialogOpen(false)} sx={{ borderRadius: 999, px: 3 }}>Verstanden</Button>
        </DialogActions>
      </Dialog>
    </>
  );
